...
class CustomCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        # Accumuler la récompense et incrémenter la longueur pour l'épisode en cours
        self.current_episode_reward += self.locals["rewards"][0]
        self.current_episode_length += 1
        
        # Vérifier si l'épisode est terminé
        if self.locals["dones"][0]:
            # Enregistrer la récompense et la longueur de l'épisode terminé
    ...
    def _on_rollout_end(self) -> None:
        # Enregistrement des statistiques à la fin du rollout
        if len(self.episode_rewards) > 0:
            ...
            # Réinitialisation pour le prochain rollout
            self.episode_rewards = []
            self.episode_lengths = []

    def _on_training_end(self) -> None:
        # Optionnel : appelé à la fin de l'entraînement
        pass
...
class CustomLunarLander(gym.Env):
    ...
    def create_world_edges(self):
        ...
        # Bords gauche et droit (étendus verticalement)
        # Bord gauche (depuis le centre du bord bas, étendu vers le haut)
        fixture = ground_body.CreateEdgeFixture(vertices=[(-half_width_m, 0), (-half_width_m, window_height_m)])
        fixture.userData = {"type":"edge"}
        # Bord droit (depuis le centre du bord bas, étendu vers le haut)
        fixture = ground_body.CreateEdgeFixture(vertices=[(half_width_m, 0), (half_width_m, window_height_m)])
        fixture.userData = {"type":"edge"}
        # Bord supérieur (étendu horizontalement de gauche à droite au sommet)
...
# Envelopper l'environnement avec VecNormalize pour la normalisation

env.reset()

if use_last_trained:
    model = PPO.load("LLCustom")
else:
